...
// Backward-compatible alias for Canon CustomFunctions2 at tag 0x0099.
const CanonCustomFunctions2 MakerNoteTag = CanonCustomFunctions

var canonMainTags = [...]MakerNoteTag{
	CanonCameraSettings,
	CanonFocalLength,
	CanonFlashInfo,
	CanonShotInfo,
...
// TagCanonIDMap maps Canon maker-note tag IDs to canonical ExifTool names.
var TagCanonIDMap = initTagCanonIDMap()

func initTagCanonIDMap() map[tag.ID]string {
	m := make(map[tag.ID]string, len(canonMainTags))
	for _, t := range canonMainTags {
		m[tag.ID(t)] = t.String()
	}
	return m
}

// TagCanonString returns the string representation of Canon maker-note tag ID.
