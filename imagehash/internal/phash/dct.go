...
func forwardDCT16(input []float32) {
	var temp [16]float32
	for i := 0; i < 8; i++ {
		x, y := input[i], input[15-i]
		temp[i] = x + y
		temp[i+8] = (x - y) / dct1632[i]
	}
	forwardDCT8(temp[:8])
	forwardDCT8(temp[8:])
	for i := 0; i < 8-1; i++ {
...
func forwardDCT8(input []float32) {
	var a, b = [4]float32{}, [4]float32{}
...
func forwardDCT4(input []float32) {
